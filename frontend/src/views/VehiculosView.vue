  ...
  <div class="p-6">
    ...
    <div v-if="activeView === 'grid'">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h2 class="text-2xl font-bold text-slate-100">Gestión de Vehículos</h2>
        <div class="flex gap-4 w-full sm:w-auto">
          <input 
            v-model="searchQuery" 
            type="text" 
            placeholder="Buscar vehículo..." 
            class="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-cyan-500/40 focus:border-cyan-500 w-full sm:w-64"
          />
      ...
      <div v-if="loading" class="text-center py-8 text-slate-400">
        <svg class="animate-spin h-8 w-8 text-cyan-400 mx-auto mb-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
        Cargando vehículos...
      </div>

      <div v-else-if="filteredVehicles.length === 0" class="text-center py-12 text-slate-400 bg-slate-800/60 rounded-xl border border-slate-700/50">
        ...
        <p class="mt-4 text-lg font-medium text-slate-100">No hay vehículos registrados</p>
        <p class="mt-1 text-sm text-slate-400">Agrega un nuevo vehículo para comenzar.</p>
      </div>

      <!-- Grid de Vehículos -->
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <div v-for="v in filteredVehicles" :key="v.id" 
             class="bg-slate-800/60 rounded-xl border border-slate-700/50 overflow-hidden shadow-md shadow-black/10 hover:shadow-lg shadow-black/20 transition-shadow cursor-pointer group flex flex-col"
             @click="openHistory(v)">
          ...
          <div class="p-5 flex-grow flex flex-col justify-between">
            <div>
              <h3 class="text-lg font-bold text-slate-100">{{ v.marca }} {{ v.modelo }} <span v-if="v.anio" class="text-slate-400 text-sm font-normal">({{ v.anio }})</span></h3>
              <p class="text-sm text-slate-400 mt-1 flex items-center gap-1">
                <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                {{ v.cliente_nombre }}
              </p>
            </div>
            <div class="mt-4 pt-4 border-t border-slate-700/40 flex items-center justify-between">
              <span class="text-xs text-slate-400">VIN: {{ v.vin || 'N/A' }}</span>
              <span class="text-cyan-400 text-sm font-medium flex items-center gap-1 group-hover:text-cyan-300">
                Ver Historial
    ...
    <div v-else-if="activeView === 'history' && selectedVehicle">
      ...
      <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div class="flex items-center gap-4">
          <button @click="closeHistory" class="p-2 bg-slate-800/50 hover:bg-slate-700 text-slate-300 rounded-full transition-colors">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
          </button>
          <div>
            <h2 class="text-2xl font-bold text-slate-100 flex items-center gap-3">
              {{ selectedVehicle.marca }} {{ selectedVehicle.modelo }}
              <span class="bg-cyan-500/15 text-cyan-300 text-sm font-bold px-2.5 py-0.5 rounded uppercase">{{ selectedVehicle.placa }}</span>
            </h2>
            <p class="text-sm text-slate-400">Propietario: {{ selectedVehicle.cliente_nombre }}</p>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
          <div class="bg-slate-800/60 rounded-xl border border-slate-700/50 overflow-hidden shadow-sm">
            <div class="h-48 bg-slate-700">
              ...
              <div v-else class="w-full h-full flex items-center justify-center text-slate-500 bg-slate-800/50">Sin foto</div>
            </div>
            <div class="p-5">
              <h3 class="text-lg font-semibold text-slate-100 mb-4">Detalles del Vehículo</h3>
              <dl class="space-y-3 text-sm">
                <div class="flex justify-between border-b border-slate-700/40 pb-2">
                  <dt class="text-slate-400">Año</dt>
                  <dd class="font-medium text-slate-100">{{ selectedVehicle.anio || 'No registrado' }}</dd>
                </div>
                <div class="flex justify-between border-b border-slate-700/40 pb-2">
                  <dt class="text-slate-400">Color</dt>
                  <dd class="font-medium text-slate-100">{{ selectedVehicle.color || 'No registrado' }}</dd>
                </div>
                <div class="flex justify-between border-b border-slate-700/40 pb-2">
                  <dt class="text-slate-400">VIN / Chasis</dt>
                  <dd class="font-medium text-slate-100">{{ selectedVehicle.vin || 'N/A' }}</dd>
                </div>
                <div class="flex justify-between pt-1">
                  <dt class="text-slate-400">Registrado en sistema</dt>
                  <dd class="font-medium text-slate-100">{{ new Date(selectedVehicle.created_at).toLocaleDateString() }}</dd>
                </div>
              </dl>
            </div>
          </div>
        ...
        <div class="lg:col-span-2">
          <div class="bg-slate-800/60 rounded-xl border border-slate-700/50 shadow-md shadow-black/10 flex flex-col h-full">
            <!-- Tabs Nav -->
            <div class="border-b border-slate-700/50 px-6 pt-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                <button
                  @click="activeHistoryTab = 'ordenes'"
                  :class="[
                    activeHistoryTab === 'ordenes' ? 'border-cyan-500 text-cyan-400' : 'border-transparent text-slate-400 hover:text-slate-300 hover:border-slate-600',
                ...
                <button
                  ...
                  :class="[
                    activeHistoryTab === 'diagnosticos' ? 'border-cyan-500 text-cyan-400' : 'border-transparent text-slate-400 hover:text-slate-300 hover:border-slate-600',
                    'whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors'
                  ]"
                >
              ...
              <div class="pb-2 sm:pb-0 w-full sm:w-64">
                <input 
                  ...
                  class="w-full px-3 py-1.5 text-sm border rounded-md focus:ring-1 focus:ring-cyan-500/40 focus:border-cyan-500 bg-slate-800/40"
                />
              </div>
            </div>

            <!-- Contenido de Tabs -->
            <div class="p-6 flex-grow bg-slate-800/40/50">
              
              <!-- Tab: Órdenes de Trabajo -->
              <div v-if="activeHistoryTab === 'ordenes'" class="space-y-4">
                <div v-if="loadingHistory" class="text-center py-12 text-slate-400">Cargando historial de órdenes...</div>
                <div v-else-if="historialFiltrado.length === 0" class="text-center py-12 text-slate-400 bg-slate-800/60 rounded-lg border border-slate-700/40">
                ...
                <div v-else class="space-y-4">
                  <div v-for="ot in historialFiltrado" :key="ot.id" class="bg-slate-800/60 border border-slate-700/50 rounded-lg p-5 relative overflow-hidden hover:shadow-md shadow-black/10 transition-shadow">
                    <div class="absolute left-0 top-0 bottom-0 w-1.5" :class="getBorderColor(ot.estado)"></div>
                    <div class="flex justify-between items-start mb-3">
                      <div>
                        <div class="flex items-center gap-3 mb-1">
                          <span class="font-bold text-slate-100 bg-slate-800/50 px-2 py-0.5 rounded text-sm tracking-wide">OT #{{ ot.numero_ot }}</span>
                          <span class="px-2.5 py-0.5 bg-slate-800/50 text-slate-300 rounded-full text-xs font-semibold" :class="getBadgeColor(ot.estado)">
                            {{ formatEstado(ot.estado) }}
                          </span>
                        </div>
                        <span class="text-xs text-slate-400">{{ formatDate(ot.created_at) }}</span>
                      </div>
                    ...
                    <div class="mb-4 text-sm text-slate-300">
                      <span class="font-semibold text-slate-100">Problema:</span> {{ ot.descripcion_problema }}
                    </div>
                    
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-slate-700/40">
                      <div>
                        <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Mecánicos Asignados</div>
                        <div v-if="ot.mecanicos && ot.mecanicos.length > 0" class="flex flex-wrap gap-1">
                          <span v-for="(m, i) in ot.mecanicos" :key="i" class="text-xs bg-slate-800/50 text-slate-300 px-2 py-1 rounded">
                            {{ m.nombre }}
                          </span>
                        </div>
                        <span v-else class="text-xs text-slate-500 italic">No asignado</span>
                      </div>
                      <div>
                        <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Repuestos ({{ ot.repuestos_count }})</div>
                        <ul v-if="ot.repuestos && ot.repuestos.length > 0" class="text-xs space-y-1">
                          <li v-for="(rep, i) in ot.repuestos" :key="i" class="text-slate-400 flex justify-between border-b border-gray-50 pb-1">
                            <span>{{ rep.nombre }}</span>
                            <span class="font-medium text-slate-100">x{{ rep.cantidad }}</span>
                          </li>
                        </ul>
                        <span v-else class="text-xs text-slate-500 italic">Ninguno</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div v-if="activeHistoryTab === 'diagnosticos'" class="space-y-4">
                <div v-if="loadingDiag" class="text-center py-12 text-slate-400">Cargando diagnósticos...</div>
                <div v-else-if="diagnosticosFiltrados.length === 0" class="text-center py-12 text-slate-400 bg-slate-800/60 rounded-lg border border-slate-700/40">
                ...
                <div v-else class="space-y-4">
                  <div v-for="d in diagnosticosFiltrados" :key="d.id" class="bg-slate-800/60 border border-slate-700/50 rounded-xl overflow-hidden shadow-sm">

                    <!-- Cabecera de la tarjeta -->
                    <div class="flex justify-between items-center px-5 py-3 bg-blue-50 border-b border-blue-100">
                      <div class="flex items-center gap-3">
                        <span class="font-bold text-sm bg-slate-800/60 border border-blue-200 text-blue-800 px-2 py-0.5 rounded tracking-wide">OT #{{ d.numero_ot || d.orden_id }}</span>
                        <span class="text-xs text-slate-400">{{ formatDate(d.created_at || d.fecha) }}</span>
                      </div>
                      <div class="text-xs text-slate-400">
                        <span class="text-slate-500">Mecánico:</span>
                        <span class="font-medium text-slate-200 ml-1">{{ d.mecanico_nombre }}</span>
                      </div>
                    </div>
                    ...
                    <div class="p-5 space-y-4">
                      ...
                      <div>
                        <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
                          ...
                          <span class="ml-1 bg-slate-700 text-slate-400 px-1.5 py-0.5 rounded-full font-bold">{{ getDtcs(d).length }}</span>
                        </div>
                        ...
                        <div v-else class="space-y-2">
                          ...
                          >
                            ...
                            <div class="flex-1 min-w-0">
                              <p class="text-sm font-semibold text-slate-200 leading-snug">
                                {{ dtc.descripcion || 'Descripción no disponible' }}
                              </p>
                              <p v-if="dtc.sistema" class="text-xs text-slate-400 mt-0.5">
                      ...
                      <div class="bg-slate-800/40 border border-slate-700/40 rounded-lg p-3">
                        <div class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Observaciones del Mecánico</div>
                        <p v-if="d.observaciones && d.observaciones !== 'Sin observaciones'" class="text-sm text-slate-200 italic leading-relaxed">"{{ d.observaciones }}"</p>
                        <p v-else class="text-sm text-slate-500 italic">Sin observaciones registradas</p>
                      </div>

                      <!-- Trama RAW (expandible) -->
                      <div v-if="d.tramas_hex" class="border border-slate-700/50 rounded-lg overflow-hidden">
                        <button
                          @click="expandedTrama = expandedTrama === d.id ? null : d.id"
                          class="w-full flex justify-between items-center px-3 py-2 bg-slate-800/40 hover:bg-slate-800/50 transition-colors text-left"
                        >
                          <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Trama RAW OBD-II</span>
                          <svg class="w-4 h-4 text-slate-500 transition-transform" :class="{ 'rotate-180': expandedTrama === d.id }" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function getBadgeColor(estado) {
  ...
  return map[estado] || 'bg-slate-800/50 text-slate-200'
}
